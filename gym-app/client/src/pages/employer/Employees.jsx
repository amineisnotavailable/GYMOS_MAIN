...
const Employees = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const res = await api.get('/employer/employees')
      ...
      <div className="glass p-6 rounded-2xl mb-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input
            ...
            required
            className="p-3 rounded-xl bg-transparent border border-white/10 text-white"
          />
          <select
            value={form.position}
          ...
          <button
            type="submit"
            className="py-3 bg-red-600 hover:bg-red-500 rounded-xl font-semibold"
          >
            Hire
          </button>
        </form>
