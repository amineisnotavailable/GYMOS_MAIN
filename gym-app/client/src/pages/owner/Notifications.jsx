...
const Notifications = () => {
  ...
  useEffect(() => {
    api.get('/owner/notifications').then(r => setNotifs(r.data))
    api.get('/owner/users').then(r => setUsers(r.data))
  }, [])

  const send = async (e) => {
    e.preventDefault()
    try {
      await api.post('/owner/notifications', { recipientId: parseInt(recipientId), message })
      setStatus('Sent')
      setMessage('')
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass p-6 rounded-2xl">
        ...
        {notifs.map(n => (
          <div key={n.id} className={`p-4 rounded-xl flex justify-between ${n.isRead ? 'opacity-50' : 'bg-white/5'}`}>
            <div>
              <p className="text-white">{n.message}</p>
              <p className="text-xs text-gray-400">{n.type} · {new Date(n.createdAt).toLocaleString()}{n.sender ? <span className="text-red-400"> · From: {n.sender.firstName} {n.sender.lastName}</span> : ''}</p>
            </div>
            {!n.isRead && <button onClick={() => markRead(n.id)} className="text-sm text-red-400">Mark read</button>}
          </div>
        ))}
      </div>
    </div>
